...
相同的类型。用户显式传入窗口绑定的一维 INT32 `signal` tensor；通信域物化会把
该 signal buffer 保留在与 `src` 相同的 comm-domain 中，即使它没有传给用户
自定义 chip kernel。public op 当前接受 `ReduceOp.Sum`，并会拒绝预留的
`Max` / `Min` / `Prod` 变体，直到这些 lowering 落地。host builtin
lowering 路径当前仅支持 `Sum` + FP32 变体。
