...
| **Gather** | `tile.gatherb` | Gather 32-byte source blocks. Each UINT32 offset selects one block; each offset column expands to `32 / sizeof(src.dtype)` output elements, and valid shape expands identically. Offset rows contain a positive multiple of eight entries. Maps to `pto.tgatherb`. |
| - | `tile.mgather` | Gather from a GM tensor into a fresh Vec tile. `coalesce="row"` gathers complete rows through a row-major `[1,R]` INT32 index tile; `"elem"` flat-indexes individual elements. Every valid index must remain inside the source tensor (out-of-range addressing is invalid). Emits canonical `pto.mgather` with an explicit coalesce attribute. |
