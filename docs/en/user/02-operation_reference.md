...
| `assemble` | `(target: Tensor, source: Tensor, offset: Sequence[IntLike], *, atomic: AtomicType = AtomicType.None_) -> Tensor` | Write source into target at offset. Sugar (pre-SSA only): `target[i:i+H, j:j+W] = source`. `atomic=AtomicType.Add` accumulates instead of overwriting (split-K) — only valid **inside an InCore function** (typically a `pl.at(level=pl.Level.CORE_GROUP, ...)` scope) where `source` lowers to an on-chip tile and the target is a function output (global memory). Every other form is rejected at compile time: a tensor-to-tensor assemble has no store to carry the combine, an assemble into an on-chip tile has no global-memory destination, and at the orchestration level no atomic-combine instruction exists. Non-deterministic FP, target must be pre-zeroed, dtypes fp32/bf16/fp16/int32/int16/int8 (bf16 requires the Ascend910B/A2/A3 profile) |
