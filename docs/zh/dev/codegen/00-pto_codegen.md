...
**当发现代码生成中存在分析逻辑时：** 创建跟踪 Issue，在有带宽时将其重构为专用 Pass。[#814](https://github.com/hw-native-sys/pypto/issues/814) 就是一个实例：编排代码生成中的返回值到参数追踪逻辑已重构为 [`NormalizeReturnOrder`](../passes/27-normalize_return_order.md) pass。
...
- `split` 就是 pto-isa 的 `TileSplitAxis`，原样打印。`0` = 不切分，`1` / `2` = 上下 /
  ...
  `e0 == e1 + 1`。这些 extent 由
  [LowerAutoVectorSplit](../passes/22-lower_auto_vector_split.md) 物化，
  [ExpandMixedKernel](../passes/23-expand_mixed_kernel.md) 选择匹配的 code。
