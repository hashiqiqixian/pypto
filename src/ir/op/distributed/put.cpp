...
TypePtr DeducePutType(const std::vector<ExprPtr>& args,
  ...
  if (args.size() == 3) {
    ...
    CHECK(dst_offsets) << "pld.tensor.put dst_offsets must be a tuple";
    CHECK(src_offsets) << "pld.tensor.put src_offsets must be a tuple";
    CHECK(transfer_shape) << "pld.tensor.put shape must be a tuple";
    CHECK(dst_offsets->elements_.size() == dst_shape.size())
        << "pld.tensor.put dst_offsets rank must match dst rank";
    CHECK(src_offsets->elements_.size() == src_shape.size())
        << "pld.tensor.put src_offsets rank must match src rank";
    ...
    for (size_t i = 0; i < transfer_shape->elements_.size(); ++i) {
      ...
      CHECK(dim) << "pld.tensor.put shape dimensions must be static constants";
      CHECK(dim->value_ > 0) << "pld.tensor.put shape dimension " << i << " must be positive, got "
                             << dim->value_;
    }
  }
