...
void ValidateGetRegionArgs(const std::vector<ExprPtr>& args, size_t region_arg_base,
                           ...
                           std::vector<ExprPtr>* out_transfer_shape = nullptr) {
...
REGISTER_OP("pld.tensor.get")
    .set_description(
        ...
        "subregion forms. Lowers to CommRemoteOffset(ctx, peer) + addptr + make_tensor_view + "
        "partition_view (src) + partition_view (dst) + a synthesised VEC staging tile + TGET at codegen.")
