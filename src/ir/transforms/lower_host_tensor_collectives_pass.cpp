...
void CheckStaticSignalCapacity(const CallPtr& call, size_t required_slots) {
  ...
  INTERNAL_CHECK_SPAN(signal_type->shape_.size() == 1, call->span_)
  ...
  if (!extent) return;
  CHECK(extent->value_ >= static_cast<int64_t>(required_slots))
      << "LowerHostTensorCollectives: pld.tensor.allreduce signal shape[0] (" << extent->value_
      << ") must be at least the participating device count (" << required_slots << ") at "
      << call->span_.to_string();
}
...
[[nodiscard]] CallPtr MakeBuiltinCall(const CallPtr& call, const ExprPtr& device) {
  ...
  auto builtin = OpRegistry::GetInstance().CreateInternal("builtin.tensor.allreduce", call->args_, kwargs,
                                                          call->span_);
