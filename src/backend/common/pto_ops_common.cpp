...
static std::string MakeGetCodegenPTO(const CallPtr& op, codegen::CodegenBase& codegen_base) {
  auto& codegen = dynamic_cast<codegen::PTOCodegen&>(codegen_base);
  const bool has_stage = op->args_.size() == 4 || op->args_.size() == 7;
  const bool has_region = op->args_.size() == 6 || op->args_.size() == 7;
  CHECK(op->args_.size() == 3 || op->args_.size() == 4 || op->args_.size() == 6 || op->args_.size() == 7)
      << op->op_->name_ << " requires 3/6 tensor-form arguments or 4/7 tile-form arguments, got "
      << op->args_.size();
  ...
  if (!has_region) {
    ...
    const size_t region_base = has_stage ? 4 : 3;
  ...
  if (has_stage) {
    stage = codegen.GetExprAsCode(op->args_[3]);
    stage_type = codegen.GetExprTypeAnnotation(op->args_[3]);
    INTERNAL_CHECK_SPAN(!stage_type.empty(), op->span_)
        << "Internal error: " << op->op_->name_ << " stage tile " << stage
        << " has no tile_buf type annotation";
  } else {
    // Legacy direct tensor.get lowering: synthesize a VEC staging tile_buf
    // sized to the 2-D-flattened transfer.
    int64_t cols = codegen.GetConstIntValue(transfer_shape[rank - 1]);
    int64_t rows = 1;
    for (size_t i = 0; i + 1 < rank; ++i) {
      rows *= codegen.GetConstIntValue(transfer_shape[i]);
    }
    stage_type = codegen::FormatTileBufTypeString(
    ...
    stage = codegen.AllocNewTileBuf(stage_type, "tget_stage");
  }
