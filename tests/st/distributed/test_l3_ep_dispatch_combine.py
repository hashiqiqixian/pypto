...
def _build_ep_dispatch_combine_program():
    ...
    class EPDispatchCombine:
        ...
        def ep_step(
            ...
            local_row = pl.load(routes, [0, 0], [1, D])
            _ = pl.store(local_row, [0, 0], recv_x)
            pld.tensor.put(
                recv_x,
                peer=peer,
                src=send_x,
            ...
            # Combine: first publish local rows into routed_y on every rank.
            # The ready barrier prevents a peer's combine put from being
            # overwritten by this rank's local store.
            ...
            # Row 1 currently belongs to the peer's original token stream, so
            # send it back to the owner, mirroring EP combine.
            pld.tensor.put(
                routed_y,
                peer=peer,
                src=recv_x,
            ...
            pld.system.notify(combine_signal, peer=peer, offsets=[0, 0], value=1, op=pld.NotifyOp.AtomicAdd)
            pld.system.wait(signal=combine_signal, offsets=[0, 0], expected=2, cmp=pld.WaitCmp.Ge)
